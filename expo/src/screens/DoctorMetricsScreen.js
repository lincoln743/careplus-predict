...
const DoctorMetricsScreen = () => {
  ...
  const normalizePatients = (raw) => {
    ...
    return list.map((p, index) => {
      ...
      const status = String(p.status ?? p.statusLabel ?? '').toLowerCase();

      let riskLevel = 'low';
      if (status.includes('alto') || status.includes('high')) riskLevel = 'high';
      else if (status.includes('alerta') || status.includes('inativo') || status.includes('medium')) riskLevel = 'medium';
      else if (score < 60) riskLevel = 'high';
      else if (score < 80) riskLevel = 'medium';

      const online =
        status.includes('online') ||
        status.includes('ativo hoje') ||
        status.includes('active_today');
